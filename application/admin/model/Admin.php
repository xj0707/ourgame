...
class Admin extends \think\Model {
    ...
    public function getInfo($id) {
        $res = $this->field('id,username,lastloginip,lastlogintime,email,mobile,realname,openid,status,merchantId')
                ->where(array('id' => $id))
                ->find();
        if ($res) {
            $res = $res->data;
    ...
    public function getUserGroups($uid) {
        ...
        $res = db('admingroupaccess')->field('group_id')->where('uid', $uid)->select();
