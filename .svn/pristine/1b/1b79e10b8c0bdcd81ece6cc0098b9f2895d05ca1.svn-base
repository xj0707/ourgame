...
class GameMaJiangController extends CommonController{
    public function index(){
        ...
        if(count($userids)) {
            $arrId = $userids;
            $rooms=Db::name('gameroomScmj')->alias('g')->join( config('database.prefix') .'gameuser u','g.un32OwnerUserID=u.un32UserID')->where("g.un32OwnerUserID", 'in', $arrId)->order('g.un64ID', 'desc')->paginate(20,false,['query' => request()->param()]);
...
//            }
            ...
            //不存在 随便查找一个不存在的
            $rooms=Db::name('gameroomScmj')->alias('g')->join( config('database.prefix') .'gameuser u','g.un32OwnerUserID=u.un32UserID')->where("g.un32OwnerUserID", '-1')->order('g.un64ID', 'desc')->paginate(20,false,['query' => request()->param()]);
        }
        if(isset($data['xl'])){
